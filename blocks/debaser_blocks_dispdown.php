...
//  ------------------------------------------------------------------------ //
	...
	function b_debaser_dispdown_show($options) {
	...
	$block = array();
		...
		if (isset($_POST['playme']) && $_POST['playme'] == 'playdown') {

		$sql = "SELECT d.xfid, d.filename, d.link, d.approved, d.fileext, d.hits, e.mime_ext, e.mime_preselect, f.xpid, f.html_code FROM ".$xoopsDB->prefix('debaser_files')." d, ".$xoopsDB->prefix('debaser_mimetypes')." e, ".$xoopsDB->prefix('debaser_player')." f WHERE d.approved = 1 AND d.fileext = e.mime_ext AND e.mime_preselect = f.xpid AND d.hits > 0 ORDER BY d.hits DESC LIMIT 1";
	$result = $xoopsDB->query($sql);

			while ( $myrow = $xoopsDB->fetchArray($result) ) {
			$disp_down = array();
			$disp_down['id'] = $myts->makeTboxData4Show($myrow['xfid']);
			...
			$disp_down['player'] = $player;
			}

		$block['display_down'][] = $disp_down;
		}
		else {
		$emptyvar = '';
		$disp_down['donotdeletethistag'] = $emptyvar;
		$block['display_down'][] = $disp_down;
		}
		return $block;
	...
	function b_debaser_dispdown_edit($options) {
