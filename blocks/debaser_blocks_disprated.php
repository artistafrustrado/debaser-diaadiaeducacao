...
//  ------------------------------------------------------------------------ //
	...
	function b_debaser_disprated_show($options) {
	...
	$block = array();
		...
		if (isset($_POST['playme']) && $_POST['playme'] == 'playrated') {

		$sql = "SELECT d.xfid, d.filename, d.link, d.approved, d.fileext, d.rating, e.mime_ext, e.mime_preselect, f.xpid, f.html_code FROM ".$xoopsDB->prefix('debaser_files')." d, ".$xoopsDB->prefix('debaser_mimetypes')." e, ".$xoopsDB->prefix('debaser_player')." f WHERE d.approved = 1 AND d.fileext = e.mime_ext AND e.mime_preselect = f.xpid ORDER BY d.rating DESC LIMIT 1";
	$result = $xoopsDB->query($sql);

			while ( $myrow = $xoopsDB->fetchArray($result) ) {
			$disp_rated = array();
			$disp_rated['id'] = $myts->makeTboxData4Show($myrow['xfid']);
			...
			$disp_rated['player'] = $player;
			}

		$block['display_rated'][] = $disp_rated;
		}
		else {
		$emptyvar = '';
		$disp_rated['donotdeletethistag'] = $emptyvar;
		$block['display_rated'][] = $disp_rated;
		}
		return $block;
	...
	function b_debaser_disprated_edit($options) {
