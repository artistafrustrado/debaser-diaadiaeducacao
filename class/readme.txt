...
A: Unzip getID3() to a directory, then access /demo/check.php
...
implementation of getID3(), please run /demo/check.php

See /demo/scandir.php for a sample recursive scanning code that
scans every file in a given directory, and all sub-directories

See /demo/simple.php for a simple example script that scans all
files in one directory and output artist, title, bitrate and playtime

See /demo/mimeonly.php for a simple example script that scans a
...
$getID3 = new getID3;
getid3_lib::IncludeOrDie(GETID3_INCLUDEPATH.'write.php', __FILE__, $dummy);
...
/demo/check.php scanning the file(s) you're interested in to
