...
class getid3_midi
{
	...
	function getid3_midi(&$fd, &$ThisFileInfo, $scanwholefile=true) {
		...
		// shortcut
		$ThisFileInfo['mid']['raw'] = array();
		$thisfile_midi               = &$ThisFileInfo['mid'];
		$thisfile_midi_raw           = &$thisfile_midi['raw'];

		$ThisFileInfo['fileformat']          = 'mid';
		$ThisFileInfo['audio']['dataformat'] = 'mid';
		...
		if (!isset($trackdataarray) || !is_array($trackdataarray)) {
			$ThisFileInfo['error'][] = 'Sem informações desse MIDI';
			unset($thisfile_midi);
			unset($ThisFileInfo['fileformat']);
			return false;
		}
