...
class getid3_writetags
{
	...
	var $tagformats         = array();        // array of tag formats to write ('id3v1', 'id3v2.2', 'id2v2.3', 'id3v2.4', 'ape', 'vorbiscomment', 'metaflac')
	...
	function WriteTags() {
		...
		foreach ($this->tagformats as $requested_tag_format) {
			if (!in_array($requested_tag_format, $AllowedTagFormats)) {
				$this->errors[] = 'Tag format "'.$requested_tag_format.'" is not allowed on "'.@$this->ThisFileInfo['fileformat'].((@$this->ThisFileInfo['fileformat'] != @$this->ThisFileInfo['audio']['dataformat']) ? '.'.@$this->ThisFileInfo['audio']['dataformat'] : '').'" files';
				return false;
			}
		}
	...
	function FormatDataForID3v1() {
		...
		if (!empty($this->tag_data['GENRE'])) {
			foreach ($this->tag_data['GENRE'] as $key => $value) {
				if (getid3_id3v1::LookupGenreID($value)) {
