...
$cons = $xoopsDB->query("SELECT genreid, subgenreid, genretitle  FROM ".XOOPS_DB_PREFIX."_debaser_genre where subgenreid='3' order by genretitle");
...
while ( $data = $xoopsDB->fetchArray($cons) ){
  ...
  $genretitle = substr($data['genretitle'], 6);
